    ...
    <TabBarShell>
      <View className="flex-1 bg-surface pt-16">
        <View className="px-4 pb-3">
          ...
          <Text className="font-jakarta-medium text-sm text-muted">Your past rides</Text>
        </View>
        ...
        {isLoading ? (
          ...
          <View className="flex-1 items-center justify-center gap-2 px-8">
            ...
            <Text className="text-center font-jakarta-medium text-sm text-muted">
              No rides yet — your history will show up here.
            </Text>
          </View>
        ) : (
          <ScrollView
              ...
              >
                <View className="flex-row items-start justify-between gap-3">
                  <View className="flex-1 flex-row items-center gap-2">
                    ...
                    >
                      <MaterialIcons
                        name={ride.status === "COMPLETED" ? "local-taxi" : "close"}
                        size={18}
                        color={ride.status === "COMPLETED" ? "#121212" : "#D9383A"}
                      />
                    </View>
                    <View className="flex-1">
                      ...
                      <Text className="font-jakarta-medium text-xs text-muted">
                        {ride.driver ? `${ride.driver.user.name} • ` : ""}
                        {timeAgo(ride.completedAt ?? ride.requestedAt)}
                      </Text>
                    </View>
                  </View>
                  {ride.status === "COMPLETED" ? (
                    ...
                    <Text className="font-jakarta-bold text-xs uppercase text-danger">
                      Cancelled
                    </Text>
                  )}
                </View>
